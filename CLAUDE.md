...
Todo relatório em HTML criado nesta pasta deve seguir os pontos abaixo. O arquivo `relatorio-migracao-dto.html` é o **modelo de referência** — em caso de dúvida sobre como implementar algum item, replicar a solução usada nele.
...
### Convenção de nomes
Usar `kebab-case` descritivo, com prefixo indicando o tipo de documento quando útil, e **sempre incluir a data de criação do arquivo no nome**, no formato `AAAA-MM-DD` ao final (antes da extensão), ex:
- `relatorio-migracao-dto-2026-08-16.html`
- `relatorio-<assunto>-AAAA-MM-DD.html`
